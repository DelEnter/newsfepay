...
public class Msrefundorderutil {
	Logger logger = Logger.getLogger(Msrefundorderutil.class.getName());
	private String result ="";
	public String get(MSrefundorder trade){
	 ...
	 NameValuePair refundOrderNo  = new BasicNameValuePair("refundOrderNo",trade.getRefundOrderNo());
	 NameValuePair masapayOrderNo  = new BasicNameValuePair("masapayOrderNo",trade.getMasapayOrderNo());
	 NameValuePair refundAmount  = new BasicNameValuePair("refundAmount",trade.getRefundAmount());
	 NameValuePair currencyCode  = new BasicNameValuePair("currencyCode",trade.getCurrencyCode());
	 NameValuePair refundSubmitTime  = new BasicNameValuePair("refundSubmitTime",trade.getRefundSubmitTime());
	 NameValuePair callbackUrl  = new BasicNameValuePair("callbackUrl",trade.getCallbackUrl());
	 ...
	 NameValuePair signMsg  = new BasicNameValuePair("signMsg",trade.getSignMsg());
		...
		nvps1.add(refundOrderNo);
		nvps1.add(masapayOrderNo);
		nvps1.add(refundAmount);
		nvps1.add(currencyCode);
		nvps1.add(refundSubmitTime);
		nvps1.add(callbackUrl);
		
		nvps1.add(merchantOrderNo);
		nvps1.add(signMsg);
		
		try{
			Msrefundorderutil h = new Msrefundorderutil();
			logger.info("masapay退款订单接口:"+nvps1.toString());
			result = h.httpPost(nvps1,"https://open.grepay.com/masapi/refund.htm");
